...
const Events = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from('events' as any).select('*').order('date', { ascending: false });
      setEvents(data || []);
      setLoading(false);
    };
    load();
  ...
  const filtered = useMemo(() => events.filter((e: any) => statusFilter === 'all' || e.status === statusFilter), [statusFilter, events]);
      ...
      <div className="container py-8">
        ...
        <div className="mt-6">
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent><SelectItem value="all">Все</SelectItem><SelectItem value="upcoming">Предстоящие</SelectItem><SelectItem value="completed">Завершённые</SelectItem></SelectContent>
          </Select>
        </div>
        {loading ? <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(i => <Skeleton key={i} className="h-40" />)}</div> : (
            ...
            {filtered.map((event: any) => (
              <Link key={event.id} to={`/events/${event.id}`} className="group rounded-lg border bg-card p-6 transition-all hover:shadow-md hover:border-accent">
                <div className="flex items-center justify-between">
                  <Badge variant={event.status === 'upcoming' ? 'default' : 'secondary'} className={event.status === 'upcoming' ? 'gold-gradient text-accent-foreground border-0' : ''}>{statusLabel(event.status)}</Badge>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground"><Swords className="h-3.5 w-3.5" />{event.fight_count} боёв</div>
                </div>
