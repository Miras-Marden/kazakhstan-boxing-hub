...
import { p4pRankings, weightClasses } from '@/data/mockData';
      ...
      <div className="navy-gradient text-primary-foreground py-10">
        <div className="container">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-3xl font-bold md:text-4xl">Рейтинги</h1>
              <p className="mt-1 text-primary-foreground/70">Рейтинговая система KPBF REC</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <TabsContent value="p4p" className="mt-6">
            <div className="overflow-hidden rounded-lg border bg-card">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {p4pRankings.map((entry) => (
                    <tr key={entry.fighterId} className="border-b last:border-0 transition-colors hover:bg-secondary/50">
                      <td className="px-4 py-4">
                        <span className={`inline-flex h-8 w-8 items-center justify-center rounded-full font-display font-bold text-sm ${entry.rank <= 3 ? 'gold-gradient text-accent-foreground' : 'bg-secondary text-muted-foreground'}`}>
                          {entry.rank}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <Link to={`/fighters/${entry.fighterId}`} className="font-semibold text-foreground hover:text-accent transition-colors">{entry.fighterName}</Link>
                      </td>
                      <td className="hidden px-4 py-4 text-muted-foreground sm:table-cell">{entry.weightClass}</td>
                      <td className="px-4 py-4 font-mono text-muted-foreground">{entry.record}</td>
          ...
          <TabsContent value="weight" className="mt-6">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {weightClasses.map((wc) => {
                const wcFighters = p4pRankings.filter(r => r.weightClass === wc);
                return (
                  <div key={wc} className="rounded-lg border bg-card p-5">
                    ...
                    {wcFighters.length === 0 ? (
                      ...
                      <div className="mt-3 space-y-2">
                        {wcFighters.map((f, i) => (
                          <div key={f.fighterId} className="flex items-center justify-between text-sm">
                            <span className="flex items-center gap-2">
                              <span className="font-display font-bold text-muted-foreground">{i + 1}</span>
                              <Link to={`/fighters/${f.fighterId}`} className="text-foreground hover:text-accent">{f.fighterName}</Link>
                            </span>
