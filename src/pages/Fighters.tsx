...
import { fighters, weightClasses } from '@/data/mockData';
...
const Fighters = () => {
  ...
  const filtered = useMemo(() => {
    return fighters.filter((f) => {
      ...
      const matchWeight = weightFilter === 'all' || f.weightClass === weightFilter;
      const matchStatus = statusFilter === 'all' || f.status === statusFilter;
      return matchSearch && matchWeight && matchStatus;
    });
  }, [search, weightFilter, statusFilter]);

  const statusLabel = (s: string) => {
    switch (s) {
      case 'active': return 'Активен';
      case 'inactive': return 'Неактивен';
      case 'retired': return 'Завершил';
      default: return s;
    }
  };
    ...
    <Layout>
      <div className="container py-8">
        ...
        <div className="mt-6 overflow-hidden rounded-lg border bg-card">
                  ...
                  <tr key={fighter.id} className="border-b last:border-0 transition-colors hover:bg-secondary/50">
                    <td className="px-4 py-3">
                      <Link to={`/fighters/${fighter.id}`} className="flex items-center gap-3 font-medium text-foreground hover:text-accent transition-colors">
                        ...
                        <div>
                          <p>{fighter.name}</p>
                          {fighter.nameEn && <p className="text-xs text-muted-foreground">{fighter.nameEn}</p>}
                        </div>
                      </Link>
                    </td>
                    <td className="hidden px-4 py-3 text-muted-foreground md:table-cell">{fighter.city}</td>
                    <td className="px-4 py-3 text-muted-foreground">{fighter.weightClass}</td>
          ...
          {filtered.length === 0 && (
            <p className="py-8 text-center text-muted-foreground">Боксёры не найдены</p>
          )}
        </div>
      </div>
    </Layout>
