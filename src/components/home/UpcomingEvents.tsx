    ...
    <section className="bg-secondary/50 py-12">
      <div className="container">
        <div className="flex items-center justify-between">
          ...
          <Link to="/events" className="flex items-center gap-1 text-sm font-medium text-accent hover:underline">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {upcoming.map((event) => (
            <Link key={event.id} to={`/events/${event.id}`} className="group rounded-lg border bg-card p-5 transition-all hover:shadow-md hover:border-accent">
              ...
              <p className="mt-3 text-sm text-muted-foreground">{event.fightCount} поединков</p>
